        ...
        >
          ...
          <li class="cart-list-con5">
            ...
            <input
              ...
              type="text"
              value="1"
              @change="hangler('change', $event.target.value * 1, cart)"
              minnum="1"
              class="itxt"
              :value="cart.skuNum"
            />
...
export default {
  ...
  methods: {
    ...
    // 修改某一个产品的个数
    hangler: throttle(async function (type, disNum, cart) {
      // type:为了区分这三个元素 加、减、输入input
      ...
      switch (type) {
        ...
        case "minus":
          ...
          // 如果出现产品的个数小于等于1，传递给服务器的个数为0
          disNum = cart.skuNum > 1 ? -1 : 0;
          break;
        case "change":
          // 用户输入进来的最终量，如果是非法的（带有汉子|出现负数），带给服务器数字零
    ...
    async deleteCartById(cart) {
      try {
        ...
        await this.$store.dispatch("deleteCarListBySkuId", cart.id);
      } catch (error) {
        alert(error.message);
      }
      this.getData();
